...
spacex_df = pd.read_csv("./10-Applied Data Science Capstone/week 3/spacex_launch_dash.csv")
max_payload = spacex_df['Payload Mass (kg)'].max()
min_payload = spacex_df['Payload Mass (kg)'].min()

df_launch_site=spacex_df.groupby('Launch Site')
# print(df_launch_site.groups.keys())
options=[]
options.append({'label':'All Sites','value':'ALL'})
for site in df_launch_site.groups.keys():
...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        df_count=new_df.groupby('class').size().reset_index(name='cnt')
        fig = px.pie(new_df, values=df_count['cnt'], 
                     names=df_count.index,
